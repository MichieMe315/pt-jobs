...
def marketing_generator(request):
    ...
    employer_choices = [
        (str(employer.pk), employer.company_name or employer.name or f"Employer {employer.pk}")
        for employer in employers
    ]
    ...
    if form.is_valid() and request.GET.get("action") in {"preview", "download"}:
        ...
        if kind == "single":
            employer_id = form.cleaned_data["employer"]
            selected_employer = next(
                (employer for employer in employers if str(employer.pk) == str(employer_id)),
                None,
            )

            if not selected_employer:
                form.add_error("employer", "That clinic is no longer eligible.")
            else:
                card = _to_card(selected_employer)
                style = form.cleaned_data.get("single_style") or "classic"
                png = render_single_graphic(
                    card=card,
                    output_format=form.cleaned_data["output_format"],
                    style=style,
                )
                title = f"{card.name}-{style}-hiring"
        else:
            ...
            if not selected:
                ...
                cards = [_to_card(employer) for employer in selected]
        ...
        if png is not None:
            ...
            filename = slugify(title) or "marketing-graphic"
